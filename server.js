...
async function startupHttpSelfTest(){
 ...
 try{
  ...
  let css=await fetch(base+'/styles.css?v=14'),cssText=await css.text();
  ...
  let js=await fetch(base+'/app.js?v=14'),jsText=await js.text();
  ...
  let sw=await fetch(base+'/sw.js?v=14'),swText=await sw.text();
  if(!sw.ok||!swText.includes("shrimp-fins-v14")||!swText.includes('/favicon.svg?v=14'))throw Error('PWA service worker self-test failed');
  let manifest=await fetch(base+'/manifest.webmanifest'),manifestText=await manifest.text();
  if(!manifest.ok||!manifestText.includes('/favicon.svg?v=14')||!manifestText.includes('"display": "standalone"'))throw Error('PWA manifest self-test failed');
  let promo=await fetch(base+'/assets/shrimp-fins-promo.webp?v=14'),promoBytes=(await promo.arrayBuffer()).byteLength;
  ...
  let store=await fetch(base+'/assets/storefront.svg?v=14'),storeText=await store.text();
