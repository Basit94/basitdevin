...
async function startupHttpSelfTest(){
 ...
 try{
  ...
  let css=await fetch(base+'/styles.css?v=16'),cssText=await css.text();
  ...
  let js=await fetch(base+'/app.js?v=16'),jsText=await js.text();
  ...
  let sw=await fetch(base+'/sw.js?v=16'),swText=await sw.text();
  if(!sw.ok||!swText.includes("shrimp-fins-v16")||!swText.includes('/favicon.svg?v=16'))throw Error('PWA service worker self-test failed');
  let manifest=await fetch(base+'/manifest.webmanifest'),manifestText=await manifest.text();
  if(!manifest.ok||!manifestText.includes('/favicon.svg?v=16')||!manifestText.includes('"display": "standalone"'))throw Error('PWA manifest self-test failed');
  let promo=await fetch(base+'/assets/shrimp-fins-promo.webp?v=16'),promoBytes=(await promo.arrayBuffer()).byteLength;
  ...
  let store=await fetch(base+'/assets/storefront.svg?v=16'),storeText=await store.text();
