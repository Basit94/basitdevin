...
const state={data:null,lang:localStorage.getItem('sf_lang')||'ar',category:'all',search:'',cart:JSON.parse(localStorage.getItem('sf_cart')||'{}'),lastOrder:null,checkoutKey:null,checkoutFingerprint:null};
const I18N={
...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{if('caches'in window){const ks=await caches.keys();await Promise.all(ks.filter(k=>k.startsWith('shrimp-fins-')&&k!=='shrimp-fins-v11').map(k=>caches.delete(k)))}await navigator.serviceWorker.register('/sw.js?v=11',{updateViaCache:'none'})}catch{}});
