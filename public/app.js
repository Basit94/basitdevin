...
function product(id){const p=state.data?.products.find(x=>x.id===id);if(p)return p;const o=state.data?.offers.find(x=>x.id===id);return o?{...o,name_ar:o.title_ar,name_en:o.title_en,orderable:true}:null}
...
function closeAll(){['#overlay','#cartDrawer','#checkoutModal','#trackModal','#successModal','#productModal','#accountModal'].forEach(s=>$(s)?.classList.add('hidden'));document.body.classList.remove('lock')}
...
function renderOffers(){
 if(!state.data)return;const arr=(state.data.offers||[]).filter(o=>(!o.starts_at||new Date(o.starts_at)<=new Date())&&(!o.ends_at||new Date(o.ends_at)>=new Date()));
 $('#offersGrid').innerHTML=arr.map(o=>`<article class="offer-card ${photoOrigin(o).cls}-photo">${remoteImg(displayImage(o),txt(o,'title_ar','title_en'))}${photoBadge(o)}<span class="offer-price">${money(o.price)}</span><div class="offer-copy"><h3>${esc(txt(o,'title_ar','title_en'))}</h3><p>${esc(txt(o,'description_ar','description_en')||'')}</p><button class="offer-add" type="button" data-offer-add="${esc(o.id)}">${tr('add')} +</button></div></article>`).join('');hydrateRemoteImages($('#offersGrid'));$$('[data-offer-add]').forEach(b=>b.onclick=()=>addToCart(b.dataset.offerAdd));
}
...
function renderProducts(){
 ...
 <div class="product-image" data-view="${esc(p.id)}">${remoteImg(image,txt(p,'name_ar','name_en'))}${photoBadge(p)}${p.featured?`<span class="featured-badge">★ ${state.lang==='ar'?'مميز':'Featured'}</span>`:''}${calorieTag(p)}</div>
...
async function placeOrder(e){
 ...
 state.lastOrder={...j.order,phone:payload.phone};localStorage.setItem('sf_last_order',JSON.stringify(state.lastOrder));if(state.customer&&type==='delivery'&&draft.latitude!=null&&draft.longitude!=null)fetch('/api/customer/addresses',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({address:draft.address,latitude:draft.latitude,longitude:draft.longitude,label:'Delivery'})}).then(()=>loadCustomer()).catch(()=>{});state.cart={};state.checkoutKey=null;state.checkoutFingerprint=null;saveCart();$('#successNo').textContent=j.order.orderNumber;closeAll();open('#successModal');
...
function renderAccount(){
 const signed=!!state.customer;$('#accountGuest').classList.toggle('hidden',signed);$('#accountSigned').classList.toggle('hidden',!signed);
 $('#accountBtn').textContent=signed?(state.customer.name+' · Account'):'حسابي · Account';
 if(!signed){$('#savedAddresses').innerHTML='';return}
 $('#accountWelcome').textContent=state.customer.name+' · '+state.customer.phone;
 $('#accountAddresses').innerHTML=(state.addresses||[]).map(a=>`<div class="account-address"><button type="button" class="saved-address" data-address-id="${esc(a.id)}">${esc(a.label)} · ${esc(a.address)}</button><button type="button" data-address-remove="${esc(a.id)}" aria-label="Remove address">×</button></div>`).join('')||'<p>No saved addresses yet.</p>';
 $('#savedAddresses').innerHTML=(state.addresses||[]).map(a=>`<button type="button" class="saved-address" data-address-id="${esc(a.id)}">${esc(a.label)} · ${esc(a.address)}</button>`).join('');
 $('#accountOrders').innerHTML=(state.orders||[]).map(o=>`<div class="account-order"><b>${esc(o.order_no)}</b><span>${esc(statusLabel(o.status))} · ${money(o.total)}</span><button type="button" data-order-token="${esc(o.token)}">${tr('track')}</button></div>`).join('')||'<p>No orders yet.</p>';
 $$('[data-address-id]').forEach(b=>b.onclick=()=>{const a=state.addresses.find(x=>String(x.id)===b.dataset.addressId);if(!a)return;$('#checkoutForm [name="address"]').value=a.address;$('#deliveryLat').value=a.latitude;$('#deliveryLng').value=a.longitude;updatePin();if(!$('#checkoutModal').classList.contains('hidden'))toast('Address selected')});
 $$('[data-address-remove]').forEach(b=>b.onclick=async()=>{if(!confirm('Remove this saved address?'))return;const r=await fetch('/api/customer/addresses/'+encodeURIComponent(b.dataset.addressRemove),{method:'DELETE'});if(r.ok)await loadCustomer();else toast('Could not remove address',true)});
 $$('[data-order-token]').forEach(b=>b.onclick=()=>{closeAll();open('#trackModal');$('#trackToken').value=b.dataset.orderToken;$('#trackPhone').value=state.customer.phone;$('#trackForm').requestSubmit()});
}
function updatePin(){const lat=Number($('#deliveryLat').value),lng=Number($('#deliveryLng').value),valid=$('#deliveryLat').value!==''&&$('#deliveryLng').value!==''&&Number.isFinite(lat)&&Number.isFinite(lng)&&Math.abs(lat)<=90&&Math.abs(lng)<=180;$('#deliveryMapLink').classList.toggle('hidden',!valid);if(valid)$('#deliveryMapLink').href='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(lat+','+lng);$('#locationStatus').textContent=valid?lat.toFixed(6)+', '+lng.toFixed(6):'Add a precise pin for the driver'}
$('#useMapLinkBtn').onclick=()=>{try{const u=new URL($('#mapPaste').value),v=u.searchParams.get('query')||u.searchParams.get('q')||u.searchParams.get('ll')||'',match=(v.match(/(-?\d{1,2}\.\d+)\s*,\s*(-?\d{1,3}\.\d+)/)||u.pathname.match(/@(-?\d{1,2}\.\d+),(-?\d{1,3}\.\d+)/)||u.href.match(/!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/));if(!/^https?:$/.test(u.protocol)||!match)throw Error('No coordinates');const lat=Number(match[1]),lng=Number(match[2]);if(Math.abs(lat)>90||Math.abs(lng)>180)throw Error('Invalid coordinates');$('#deliveryLat').value=lat;$('#deliveryLng').value=lng;updatePin()}catch{toast('Paste a full Maps link containing a latitude and longitude.',true)}};
$('#useLocationBtn').onclick=()=>{if(!navigator.geolocation)return toast('Location is unavailable on this device.',true);$('#locationStatus').textContent='Finding your location…';navigator.geolocation.getCurrentPosition(pos=>{$('#deliveryLat').value=pos.coords.latitude.toFixed(7);$('#deliveryLng').value=pos.coords.longitude.toFixed(7);updatePin()},()=>{$('#locationStatus').textContent='Location permission denied. Enter coordinates manually or share your address.'},{enableHighAccuracy:true,timeout:12000,maximumAge:0})};$('#deliveryLat').oninput=updatePin;$('#deliveryLng').oninput=updatePin;
function setAccountMode(mode){state.accountMode=mode;$('#accountNameField').classList.toggle('hidden',mode!=='register');$('#accountNameField input').required=mode==='register';$('#accountSubmit').textContent=mode==='register'?'إنشاء حساب · Create account':'دخول · Sign in'}
$('#showLogin').onclick=()=>setAccountMode('login');$('#showRegister').onclick=()=>setAccountMode('register');$('#accountBtn').onclick=()=>open('#accountModal');$('#mobileAccountBtn').onclick=()=>open('#accountModal');$('#accountForm').onsubmit=async e=>{e.preventDefault();const form=new FormData(e.currentTarget),btn=$('#accountSubmit');btn.disabled=true;try{const r=await fetch('/api/customer/'+state.accountMode,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(Object.fromEntries(form))}),j=await r.json();if(!r.ok)throw Error(j.error||'Account request failed');await loadCustomer();$('#checkoutForm [name="customerName"]').value=state.customer.name;$('#checkoutForm [name="phone"]').value=state.customer.phone;toast('Welcome · أهلاً بك')}catch(err){toast(err.message,true)}finally{btn.disabled=false}};$('#accountLogout').onclick=async()=>{await fetch('/api/customer/logout',{method:'POST'});state.customer=null;state.addresses=[];renderAccount()};
...
$('#checkoutBtn').onclick=()=>{if(!cartQty())return toast(tr('emptyCart'),true);if(state.data.settings.acceptingOrders===false)return toast(tr('closed'),true);state.checkoutKey=state.checkoutKey||(crypto.randomUUID?crypto.randomUUID():(Date.now()+'-'+Math.random().toString(36).slice(2)));closeAll();open('#checkoutModal');if(state.customer){$('#checkoutForm [name="customerName"]').value=state.customer.name;$('#checkoutForm [name="phone"]').value=state.customer.phone}updateCheckoutTotal()};
...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{if('caches'in window){const ks=await caches.keys();await Promise.all(ks.filter(k=>k.startsWith('shrimp-fins-')&&k!=='shrimp-fins-v20').map(k=>caches.delete(k)))}await navigator.serviceWorker.register('/sw.js?v=20',{updateViaCache:'none'})}catch{}});
let loadedBuild='';
