...
function renderSettings(){
 ...
 setHref('#mapBtn','https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(st.mapQuery||st.addressAr||st.addressEn||'Shrimp Fins Riyadh'));
 ...
 if(st.heroImage&&$('#heroFoodImage'))setImage($('#heroFoodImage'),st.heroImage,'🦐');
...
async function placeOrder(e){
 ...
 const payload={customerName:form.get('customerName'),phone:form.get('phone'),orderType:type,address:form.get('address')||'',notes:form.get('notes')||'',paymentMethod:form.get('paymentMethod')||'cod',items:Object.entries(state.cart).map(([productId,qty])=>({productId,qty}))};
