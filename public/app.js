...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{if('caches'in window){const ks=await caches.keys();await Promise.all(ks.filter(k=>k.startsWith('shrimp-fins-')&&k!=='shrimp-fins-v18').map(k=>caches.delete(k)))}await navigator.serviceWorker.register('/sw.js?v=18',{updateViaCache:'none'})}catch{}});
let loadedBuild='';
