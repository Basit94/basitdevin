...
const state={data:null,lang:localStorage.getItem('sf_lang')||'ar',category:'all',search:'',cart:JSON.parse(localStorage.getItem('sf_cart')||'{}'),lastOrder:null,checkoutKey:null};
const I18N={
...
function saveCart(){state.checkoutKey=null;localStorage.setItem('sf_cart',JSON.stringify(state.cart));renderCart()}
...
async function placeOrder(e){
 ...
 state.checkoutKey=state.checkoutKey||(crypto.randomUUID?crypto.randomUUID():(Date.now()+'-'+Math.random().toString(36).slice(2)));const payload={customerName:form.get('customerName'),phone:form.get('phone'),orderType:type,address:form.get('address')||'',notes:form.get('notes')||'',paymentMethod:form.get('paymentMethod')||'cod',clientRequestId:state.checkoutKey,items:Object.entries(state.cart).map(([productId,qty])=>({productId,qty}))};
 ...
 state.lastOrder={...j.order,phone:payload.phone};localStorage.setItem('sf_last_order',JSON.stringify(state.lastOrder));state.cart={};state.checkoutKey=null;saveCart();$('#successNo').textContent=j.order.orderNumber;closeAll();open('#successModal');
