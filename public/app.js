...
function renderAccount(){const signed=!!state.customer;$('#accountGuest').classList.toggle('hidden',signed);$('#accountSigned').classList.toggle('hidden',!signed);$('#accountBtn').textContent=signed?(state.customer.name+' · Account'):'حسابي · Account';if(!signed){$('#savedAddresses').innerHTML='';return}$('#accountWelcome').textContent=state.customer.name+' · '+state.customer.phone;$('#accountAddresses').innerHTML=(state.addresses||[]).map(a=>`<button type="button" class="saved-address" data-address-id="${esc(a.id)}">${esc(a.label)} · ${esc(a.address)}</button>`).join('')||'<p>No saved addresses yet.</p>';$('#savedAddresses').innerHTML=$('#accountAddresses').innerHTML;$('#accountOrders').innerHTML=(state.orders||[]).map(o=>`<div class="account-order"><b>${esc(o.order_no)}</b><span>${esc(statusLabel(o.status))} · ${money(o.total)}</span><button type="button" data-order-token="${esc(o.token)}">${tr('track')}</button></div>`).join('')||'<p>No orders yet.</p>';$$('[data-address-id]').forEach(b=>b.onclick=()=>{const a=state.addresses.find(x=>String(x.id)===b.dataset.addressId);if(!a)return;$('#checkoutForm [name="address"]').value=a.address;$('#deliveryLat').value=a.latitude;$('#deliveryLng').value=a.longitude;updatePin()});$$('[data-order-token]').forEach(b=>b.onclick=()=>{closeAll();open('#trackModal');$('#trackToken').value=b.dataset.orderToken;$('#trackPhone').value=state.customer.phone;$('#trackForm').requestSubmit()})}
